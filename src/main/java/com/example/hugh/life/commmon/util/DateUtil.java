...
import com.example.hugh.life.commmon.enums.TimeIntervalEnum;
...
//    }
    ...
    public static Date getDateByInterval(Date now,int interval){
        if(null == now){
            now = new Date();
        }

        if(interval == TimeIntervalEnum.day.getInterval()){
            return getBeginDate(now, 1);
        }else if(interval == TimeIntervalEnum.week.getInterval()){

        }else if(interval == TimeIntervalEnum.month.getInterval()){

        }else if(interval == TimeIntervalEnum.year.getInterval()){
